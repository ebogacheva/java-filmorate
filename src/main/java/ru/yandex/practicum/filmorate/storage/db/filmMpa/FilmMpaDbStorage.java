...
public class FilmMpaDbStorage implements FilmMpaStorage{
    ...
    private static final String SQL_QUERY_GET_MPA_BY_FILM_ID = "SELECT mpa.id, mpa.name FROM film_mpa LEFT JOIN mpa ON " +
            "film_mpa.mpa_id = mpa.id WHERE film_id = ?";
    ...
    public Mpa getFilmMpaById(int filmId) {
        Mpa mpa = null;
        try {
            mpa = jdbcTemplate.queryForObject(SQL_QUERY_GET_MPA_BY_FILM_ID, MpaDbStorage::mapRowToMpa, filmId);
