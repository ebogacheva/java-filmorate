package ru.yandex.practicum.filmorate.utils;

public class Constants {

    public static final String USER_NOT_FOUND_EXCEPTION_INFO = "Пользователь не найден.";
    public static final String FILM_NOT_FOUND_EXCEPTION_INFO = "Фильм не найден.";
    public static final String GENRE_NOT_FOUND_EXCEPTION_INFO = "Жанр не найден.";
    public static final String RATING_MPA_NOT_FOUND_EXCEPTION_INFO = "Рейтинг MPA не найден.";
    public static final String FRIEND_REQUEST_NOT_FOUND_EXCEPTION_INFO =
            "Подтвердить запрос на дружбу не удалось - запрос в друзья не найден.";
    public static final String LIKE_NOT_PERFORMED_OPERATION_INFO = "Запрос не выполнен - поставить лайк фильму.";
    public static final String DISLIKE_NOT_PERFORMED_OPERATION_INFO = "Запрос не выполнен - удалить лайк.";
    public static final String ADD_FRIEND_NOT_PERFORMED_EXCEPTION_INFO = "Запрос не выполнен - добавление в друзья.";
    public static final String FRIEND_DELETE_NOT_PERFORMED_EXCEPTION_INFO = "Запрос не выполнен - удаление из друзей.";
    public static final String DELETE_NOT_PERFORMED_EXCEPTION_INFO = "Запрос не выполнен - удаление элемента из базы.";
    public static final String UPDATE_NOT_PERFORMED_EXCEPTION_INFO = "Запрос не выполнен - обновление элемента в базе.";
    public static final String ERROR_MESSAGE = "Ошибка ";
    public static final String FILM_ADDED_LOG = "Добавлен фильм {}";
    public static final String FILM_NOT_FOUND_LOG = "Фильм не найден: {}";
    public static final String GOT_FILM_BY_ID = "Получен фильм: {}";
    public static final String USERS_LIKES_LOG = "Фильм {} нравится {} пользователям.";
    public static final String UPDATED_FILM_LOG = "Обновлена информация о фильме {}.";
    public static final String DELETED_FILM_LOG = "Удален фильм: {}.";
    public static final String USER_LIKE_FILM_LOG = "Пользователю {} нравится фильм {}.";
    public static final String USER_DISLIKE_FILM_LOG = "Пользователю {} больше не нравится фильм {}.";
    public static final String GOT_GENRE_BY_ID_LOG = "Из базы получена ифнормация о жанре {}.";
    public static final String SENT_FRIEND_REQUEST_LOG = "Пользователь {} отправил запрос на дружбу пользователю {}.";
    public static final String DELETE_FROM_FRIENDS_LOG = "Пользователь {} больше не дружит с пользователем {}.";
    public static final String USER_FRIENDS_REQUESTS_LOG = "У пользователя {} - {} запросов на дружбу.";
    public static final String CONFIRM_REQUEST_LOG = "Пользователь {} подтвердил дружбу с пользователем {}.";
    public static final String USER_NOT_FOUND_LOG = "Пользователь не найден: {}";
    public static final String GOT_USER_BY_ID_LOG = "Получен пользователь: {}";
    public static final String UPDATED_USER_LOG = "Обновлен пользователь: {}";
    public static final String USER_DELETED_LOG = "Удален пользователь: {}";
    public static final String COMMON_FRIENDS_LOG = "Общие друзья {} и {}: {}.";
}
